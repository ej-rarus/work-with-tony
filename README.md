...
| [**hwp-form**](plugins/hwp-form/) | 빈 한글(HWP/HWPX) 신청서를 주면 내 정보와 자료로 채운 새 파일을 만들고, 서명·동의처럼 직접 할 일만 체크리스트로 남기기 | 지원사업·공모전 신청서를 낼 때마다 같은 정보를 반복해서 칠 때 |
...
### hwp-form: 빈 신청서를 주면 채워서 돌려주기

**빈 한글(HWP/HWPX) 신청서를 주면, 내 정보와 자료로 채운 새 파일을 만들어 줍니다.** 지원사업·공모전 신청서마다 이름, 연락처, 경력을 다시 치고 사업계획을 칸에 옮겨 붙이던 일을 대신해요.
...
1. 서식의 칸을 읽고 어떻게 채울지 목록을 먼저 보여줍니다. 확인하기 전에는 쓰지 않아요.

   | 분류 | 예 | 처리 |
   |---|---|---|
   | 바로 채움 | 성명, 생년월일, 주소, 연락처, 경력, 자격증 | 한 번 저장해 둔 프로필(`~/.hwp-form/profile.md`)에서 가져옴 |
   | 초안 작성 | 창업 동기, 아이템 개요, 홍보 전략 | 알려 준 자료로만 작성. 자료에 없는 내용은 지어내지 않고 `확인 필요` 표시 |
   | 물어봄 | □개인/□단체, 지원경로 | 직접 고른 것만 체크 |
   | 비워 둠 | 서명, (인), 개인정보 동의, 서약 체크, 제출일 | 법적인 선택이라 항상 직접 |

2. 확인하면 원본 옆에 `신청서-작성본-20260925.hwpx`를 새로 만듭니다. **원본은 절대 건드리지 않아요.**
3. 남은 일을 체크리스트로 알려 줍니다. 예: "서명·도장 2곳, 동의 체크 1곳, 첨부서류 2종, 유의사항 삭제 후 제출".
4. 한컴오피스에서 열어 검토하고 서명한 뒤 제출하면 끝입니다.

빈 칸, 병합 셀, 표 안의 표처럼 일반 HWP 도구가 조용히 틀리는 부분은 전용 스크립트로 처리하고, 채운 값은 다시 읽어서 확인해요. `.hwp` 파일은 한컴오피스에서 HWPX로 한 번 다른 이름으로 저장해서 주면 됩니다.
